...
tags: ["Development"]
excerpt: "There are a bunch of articles on how to setup Apollo GraphQL in Nextjs and a github repository from NextJs docs. Great starting points particularly for query..."
---
...
The call to creating the Apollo client on the server is important for querying database resources. Since the schema and methods are located locally we can make use of the 
```
makeExecutableSchema
```
 method from graphQL tools and pass in the schema and resolvers.
...
Other resources including the article [from Kellen Mace](https://developers.wpengine.com/blog/apollo-client-cache-rehydration-in-next-js) is really useful for querying remote API's externally to our Next App.
