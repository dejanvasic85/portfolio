...
const wpExportPath = path.join(__dirname, 'data/softwareengineeringnotebook.WordPress.2025-04-28.xml');
...
for (let i = 0; i < items.length; i++) {
  ...
  content = content.replace(/\[gist\s+https:\/\/gist\.github\.com\/([^\/]+)\/([^\]]+)\]/g, (match, user, gistId) => {
    return `<iframe src="https://gist.github.com/${user}/${gistId}.pibb" width="100%" frameborder="0"></iframe>`;
  });
  
  // Handle [code] shortcodes
  content = content.replace(/\[code\s+language="([^"]+)"\]([\s\S]*?)\[\/code\]/g, (match, lang, code) => {
    return `\n\`\`\`${lang}\n${code}\n\`\`\`\n`;
  });
